...
function setup() {
  ...
  const samurai: any = makePlayer('samurai', 'Samurai', 13, 600, 600, 1);
  const enemy: any = makePlayer('enemy', 'Enemy', 1, 860, 600, 2);
  const ally: any = makePlayer('ally', 'Ally', 0, 880, 600, 1);
  samurai.facing = 0;
...
describe('Samurai iaijutsu duel', () => {
  it('keeps Samurai as player character id 13', () => {
    const samurai: any = getCharacter(13);
    expect(samurai.id).toBe(13);
    expect(samurai.ultimate.type).toBe('samurai_iaijutsu');
  });

  it('schedules three delayed slashes in order', () => {
    const { state, samurai } = setup();
    executeAction(state, samurai, getCharacter(13).ultimate);
