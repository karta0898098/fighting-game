...
function setup() {
  ...
  const star: any = makePlayer('star', 'Star', 18, 600, 600, 1);
  const enemy: any = makePlayer('enemy', 'Enemy', 5, 760, 600, 2);
  const ally: any = makePlayer('ally', 'Ally', 1, 760, 620, 1);
  enemy.maxHp = 500;
  enemy.hp = 500;
...
describe('Star Orbit character hook', () => {
  ...
  it('guard grants shield through post effects and refills shards', () => {
    ...
    executeAction(state, star, getCharacter(18).skill2);
    expect(star.starOrbit.shards).toBe(3);
  ...
  it('cannon spends shards and scales its damage', () => {
    ...
    executeAction(state, star, getCharacter(18).skill1);
  ...
  it('burst creates three beam pulses and damages enemies only', () => {
    const { state, star, enemy, ally } = setup();
    star.facing = 0;
    executeAction(state, star, getCharacter(18).ultimate);
