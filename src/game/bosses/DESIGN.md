...
```js
switch (a.type) {
  case 'melee':      return d <= range + 60;
  case 'charge':     return d >= 90 && d <= range + 40;
  case 'leap':       return d >= 60 && d <= range + 40;
  case 'blink':      return d <= range + 80;
  case 'projectile': return d <= (range || speed * lifetime || 760);
  case 'zone':       return range === 0 ? true : d <= range + radius + 40;
  default:           return true; // buff/召喚/自訂 → 隨時可用
}
```
...
phases: [
  {
    ...
    tagsOverride: [         // （選用）UI 提示標籤
      { icon: '⚡', text: '攻擊強化 +20%' }
    ],
    onEnter: (state, boss) => {  // （選用）觸發腳本
